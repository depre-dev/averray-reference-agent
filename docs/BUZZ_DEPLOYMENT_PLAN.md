...
The two are worth running separately: the first proves the relay booted, the
second proves the tunnel route and the shared-network attachment. If the first
passes and the second does not, the fault is networking, not Buzz.
