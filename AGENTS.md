...
- **Branch workers** (`codex-branch-worker`, `claude-branch-worker`) are the
  ...
  drawer, and drops only an unsupported Claude CLI flag on retry if the installed
  CLI rejects one of those flags. The outer task-runner timeout is the execution
  bound. The Codex worker iterates an existing PR.
...
3. **INT-2 supervised dispatch** in a separate required job: real dispatcher,
   two disposable Postgres databases, Harness pin `73133ef`, Docker-isolated
