...
export const FIXTURE_CARDS: BoardCard[] = [
  ...
  // ── Browser mission (testbed) — runs inside Hermes checking lane ──
  {
    ...
    mission: {
      ...
      verdictTone: "warn",
      confidence: 0.81,
      goal: "Verify a fresh agent can connect a wallet, claim work, and reach the first receipt without project-specific memory.",
      conclusion: "PARTIAL — Sign-message modal latency slowed the claim path.",
      agentNarrative: [
        "Opened a clean Chromium context at https://staging.averray.com/onboarding.",
        "Clicked Connect wallet and waited for the sign-message modal.",
        "Stopped after the receipt rendered because the read-only mission boundary held.",
      ],
      latency: "2m 14s",
