...
|---|---|---|---|---|
| Dispatcher crash before submit | `absent` | `proven` | Amended 2026-08-06: the first process crashed with a generation-1 claim and zero Harness runs; after expiry the second process acquired generation 2, submitted the same derived id, and produced one attempt-1 run plus one binding. [D0 silence](evidence/INT4C_D0_PARTIAL_2026-08-06.md); [INT-4c drill evidence](evidence/INT4C_LEASE_TAKEOVER_DRILLS_2026-08-06.md). | Dispatcher operator |
| Dispatcher crash after submit before binding write | `proven` | `proven` | **Restart suite case (exactly-once held).** Amended 2026-08-06: a two-process takeover additionally found the already persisted derived run, replayed it idempotently, retained one attempt-1 run, and wrote the binding as the new holder. [INT-4c drill evidence](evidence/INT4C_LEASE_TAKEOVER_DRILLS_2026-08-06.md). | Dispatcher operator |
...
|---|---|---|
| Durable lease expiry | **Column exists, written `null` — `dispatch-claim.ts:155`. Schema-present, semantics-absent.** | `proven` on 2026-08-06: claims have a finite lease, progress-only renewal, one generation-2 takeover, and critical generation-2 exhaustion. Both crash points and the live-holder negative ran through real processes and disposable stores. [D0 baseline](evidence/INT4C_D0_PARTIAL_2026-08-06.md); [INT-4c evidence](evidence/INT4C_LEASE_TAKEOVER_DRILLS_2026-08-06.md). |
...
| Queue backpressure | **Absent.** | `proven` on 2026-08-06: INT-4c introduces the first in-flight bound; exact main had none. At one non-terminal bound run, a queued task received operator-visible reason `backpressure`, one warn alert across repeated cycles, and dispatched after the bound cleared. [D0 baseline](evidence/INT4C_D0_PARTIAL_2026-08-06.md); [INT-4c evidence](evidence/INT4C_LEASE_TAKEOVER_DRILLS_2026-08-06.md). |
