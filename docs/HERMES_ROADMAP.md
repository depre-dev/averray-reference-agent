...
| C3 | Specialist agents (test-writer/security/docs) | ✅ first specialist done — `test-writer` specialist template (#310; `services/slack-operator/src/specialist-agents.ts`) + off-by-default runner wiring (#311; `ops/compose.yml`, `test/unit/test-writer-runner.test.ts`). Security/docs specialists are in review in the current C3 extension PR; do not mark complete until merged/deployed |
...
*(Status as of 2026-06-01 — **shipped/code-backed:** O0–O5, A1–A4, D1–D4, B1 first slices, B2 proposes-only self-healing, C1 first slice, C2, C3 test-writer slice, C4 v1, T1–T3, T5, T6 first slice, and the T7 reference-agent manifest. **In progress:** T4 live gold-path driver wiring and the C3 security/docs specialist extension. **Design / follow-up:** B1 idle-triggered auto-flow, C1 automatic/default reviewer dispatch, T4 hosted run proof, and the T7 platform helper. Merge/deploy remain human-gated; autopilot approves dispatch only inside O4 guardrails.)*
