...
export interface HarnessProjectionBinding {
  workItemId: string;
  ...
  manifest: AgentRunProjectionV1["manifest"];
  budget: {
    elapsedSecondsLimit?: number;
    modelTokensLimit?: number;
    toolCallsLimit?: number;
    estimatedUsdMicrosLimit?: number | null;
  };
  averrayJobId?: string;
  averraySessionId?: string;
  pullRequest?: NonNullable<AgentRunProjectionV1["bindings"]>["pullRequest"];
}

export interface HarnessProjectionRegistry {
  bindings: HarnessProjectionBinding[];
}

type HarnessRunBinding = HarnessProjectionBinding;
type HarnessRunRegistry = HarnessProjectionRegistry;
